...
function iniciar_contexto_index(){
   ...
   agregarEvento(_btnContactoDos,"click",suscribirse);
...
function redireccionar_usuario(d){
    if(d.respuesta){
        ...
        var us={
            ...
            documento:v[0].DocumentoUsuario
        };
    ...
    console.log(this.id);
     if(this.id=="btnContactar"){
            ...
            if(vf){
                ...
                if(vf.Otros[0]!=""){
                    var datos={nombre_contacto:vf.Texto[0],telefono:vf.Texto[1],correo:vf.Email[0],observaciones:vf.Otros[0]};
                //Invoco mi funcion 
                registrarDato(_contextoIngresar,"contactar",datos,mostrarMensaje,_formContactar);
                }else{
                    mostrarMensaje("Dejanos un mensaje para saber como ayudarte");
                }
            }else{
...
function suscribirse(){
    console.log(this.id);
     if(this.id=="btnContactar2"){
            var vf=obtener_valores_formulario(_formContactarDos);   
            console.log(vf);
            if(vf){
                //Creo el objeto que voy a enviar con datos a la peticion
                if(vf.Email[0]!=""){
                    var datos={correo:vf.Email[0]};
                //Invoco mi funcion 
                registrarDato(_contextoIngresar,"suscripcion",datos,mostrarMensaje,_formContactarDos);
                }else{
                    mostrarMensaje("Necessaitas un correo para suscribirte");
                }
            }else{
               mostrarMensaje({mensaje:"por favor ingresa valores"});
            }   
...
function dibujar_archivos_descarga(datos){
    ...
    if(datos.respuesta){
        ...
        var tabla=document.getElementById("tablaDescargas");
        tabla.innerHTML="";
        
        var fila=document.createElement("tr");
        fila.className="titDescarga";

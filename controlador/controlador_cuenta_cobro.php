...
if(isset($_POST['datos'])){
    ...
    $objeto= new CuentaCobro();//Mi clase  modelo 
    ...
    switch($operacion){
        case "crear":
            ...
            $objeto->codigo_cuenta_cobro=count($objeto->filas)+1;
            $objeto->fecha_cuenta_cobro=trim($post->hora_cliente);
            ...
            if($r["respuesta"]){
                ...
                $p=0;$s=0;
                $arrS=array();
                foreach ($post->datos->lista_cuenta_cobro as $key => $value) {
                    if($value->tipo=="servicio"){
                        $arrP[$p]=$objeto->crear_registro_detalle_cuenta_cobro_servicio($value->id_servicio, $value->cantidad, $value->precio);
                        $p++;
                    }else{
                       $arrS[$s]=$objeto->crear_registro_detalle_cuenta_cobro_producto($value->id_producto, $value->cantidad, $value->precio);
                       $s++;
